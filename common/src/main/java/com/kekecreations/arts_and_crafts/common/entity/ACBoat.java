...
public class ACBoat extends Boat implements ACBoatAccessor {
    public ACBoat(EntityType<? extends Boat> p_478759_, Level p_478717_, Supplier<Item> p_481824_) {
    ...
    public ACBoat(EntityType<? extends Boat> type, Level level, Supplier<Item> itemSupplier) {
    ...
    protected void readAdditionalSaveData(CompoundTag nbt) {
        if (nbt.contains("wood_type", Tag.TAG_STRING)) {
            this.entityData.set(DATA_ID_BOAT_WOOD_TYPE, WoodType.byName(nbt.getString("wood_type")).ordinal());
        }
    }

    @Override
    public Item getDropItem() {
        return switch (WoodType.byId(this.entityData.get(DATA_ID_BOAT_WOOD_TYPE))) {
    ...
    public enum WoodType implements StringRepresentable {
        ...
        public static final EnumCodec<WoodType> CODEC;
        ...
        static {
            ...
            BY_ID = ByIdMap.continuous(Enum::ordinal, WoodType.values(), ByIdMap.OutOfBoundsStrategy.ZERO);
        }
    }
